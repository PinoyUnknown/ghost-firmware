...
def readProps(prefsLoc):
    ...
    version_suffix = version.get("suffix", "").strip()
    short_version = "{}.{}.{}".format(version["major"], version["minor"], version["build"])
    if version_suffix:
        short_version = "{}-{}".format(short_version, version_suffix)
    verObj = dict(
        short=short_version,
        long="unset",
        deb="unset",
    ...
    try:
        ...
        #     suffix = sha + "-d"
        verObj["long"] = verObj["short"] if version_suffix else "{}.{}".format(verObj["short"], suffix)
