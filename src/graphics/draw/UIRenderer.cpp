...
void UIRenderer::drawBootIconScreen(const char *upperMsg, OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    ...
    static_cast<void>(upperMsg);
    static_cast<void>(state);
    setAndRegisterTFTColorRole(TFTColorRole::BootSplash, TFTPalette::GhostCyan, TFTPalette::Black, x, y, SCREEN_WIDTH,
                               SCREEN_HEIGHT);
    display->clear();
    display->setColor(WHITE);
    display->drawRect(x + 4, y + 4, SCREEN_WIDTH - 8, SCREEN_HEIGHT - 8);
    display->drawLine(x + 10, y + 20, x + 43, y + 20);
    display->drawLine(x + SCREEN_WIDTH - 43, y + SCREEN_HEIGHT - 20, x + SCREEN_WIDTH - 10, y + SCREEN_HEIGHT - 20);
    display->setTextAlignment(TEXT_ALIGN_CENTER);
    display->setFont(FONT_LARGE);
    display->drawString(x + SCREEN_WIDTH / 2, y + 32, "GHOST");
    display->drawString(x + SCREEN_WIDTH / 2 + 1, y + 32, "GHOST");
    display->setFont(FONT_SMALL);
    display->drawString(x + SCREEN_WIDTH / 2, y + 72, "PinoyUnknown");
    char version[40];
    snprintf(version, sizeof(version), "v%s", xstr(APP_VERSION_SHORT));
    display->drawString(x + SCREEN_WIDTH / 2, y + 99, version);
    display->setTextAlignment(TEXT_ALIGN_LEFT);
    screen->forceDisplay();

    setAndRegisterTFTColorRole(TFTColorRole::BootSplash, TFTPalette::GhostYellow, TFTPalette::Black, x, y, SCREEN_WIDTH,
                               SCREEN_HEIGHT);
    screen->forceDisplay();
    delay(70);
    setAndRegisterTFTColorRole(TFTColorRole::BootSplash, TFTPalette::GhostCyan, TFTPalette::Black, x, y, SCREEN_WIDTH,
