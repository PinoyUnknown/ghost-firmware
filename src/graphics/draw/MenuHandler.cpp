...
#include "platform/extra_variants/m5stack_cardputer_adv/GhostAudio.h"
#endif
...
constexpr uint8_t kMaxWifiSurveyEntries = 10;
WifiSurveyEntry wifiSurveyEntries[kMaxWifiSurveyEntries]{};
uint8_t wifiSurveyCount = 0;

const char *wifiSecurityName(wifi_auth_mode_t authMode)
{
    switch (authMode) {
    case WIFI_AUTH_OPEN:
        return "Open";
...
void menuHandler::systemBaseMenu()
{
    ...
    optionsEnumArray[options++] = WiFiScan;
    optionsArray[options] = "Network Info";
    optionsEnumArray[options++] = WiFiInfo;
#endif

    optionsArray[options] = "About / Developers";
    optionsEnumArray[options++] = About;
    ...
    bannerOptions.bannerCallback = [](int selected) -> void {
        if (selected == Notifications) {
            ...
            screen->runNow();
        } else if (selected == WiFiScan) {
            menuQueue = WifiScanMenu;
            screen->runNow();
        } else if (selected == WiFiInfo) {
            menuQueue = WifiNetworkInfoMenu;
            screen->runNow();
#endif
        } else if (selected == About) {
            menuQueue = GhostAboutMenu;
            screen->runNow();
...
void menuHandler::ghostAboutMenu()
{
    static const char *optionsArray[] = {"Back", "PinoyUnknown", "CyberGhost", "M5Burner Developer"};
    BannerOverlayOptions bannerOptions;
    bannerOptions.message = "Ghost Developers";
    bannerOptions.optionsArrayPtr = optionsArray;
    bannerOptions.optionsCount = 4;
    bannerOptions.bannerCallback = [](int selected) -> void {
        if (selected == 1) {
            screen->showSimpleBanner("PinoyUnknown\nhttps://github.com/PinoyUnknown/", 6000);
        } else if (selected == 2) {
            screen->showSimpleBanner("CyberGhost\nhttps://github.com/unidentifiedcyberghost", 6000);
        } else if (selected == 3) {
            screen->showSimpleBanner("M5Burner developer\nburner.m5stack.com/developers\nID: 2102783541171187713", 7000);
        }
    };
    screen->showOverlayBanner(bannerOptions);
}
...
void menuHandler::wifiScanMenu()
{
    ...
    if (previousMode == WIFI_MODE_NULL && !WiFi.mode(WIFI_STA)) {
        LOG_ERROR("Ghost WiFi scan could not start station mode");
        screen->showSimpleBanner("WiFi scan failed\nCould not start radio", 4000);
        return;
    }

    const int16_t found = WiFi.scanNetworks(false, true, true);
    if (found < 0) {
        WiFi.mode(previousMode);
        WiFi.scanDelete();
        LOG_ERROR("Ghost WiFi scan failed: %d", found);
        screen->showSimpleBanner("WiFi scan failed", 4000);
        return;
    }

    wifiSurveyCount = static_cast<uint8_t>(std::min<int16_t>(found, kMaxWifiSurveyEntries));
    for (uint8_t i = 0; i < wifiSurveyCount; ++i) {
        const String ssid = WiFi.SSID(i);
        if (ssid.isEmpty()) {
            strlcpy(wifiSurveyEntries[i].ssid, "(hidden)", sizeof(wifiSurveyEntries[i].ssid));
        } else {
            strlcpy(wifiSurveyEntries[i].ssid, ssid.c_str(), sizeof(wifiSurveyEntries[i].ssid));
        }

        const uint8_t *bssid = WiFi.BSSID(i);
        if (bssid) {
            snprintf(wifiSurveyEntries[i].bssid, sizeof(wifiSurveyEntries[i].bssid), "%02X:%02X:%02X:%02X:%02X:%02X",
                     bssid[0], bssid[1], bssid[2], bssid[3], bssid[4], bssid[5]);
        } else {
            strlcpy(wifiSurveyEntries[i].bssid, "unavailable", sizeof(wifiSurveyEntries[i].bssid));
        }
        strlcpy(wifiSurveyEntries[i].security, wifiSecurityName(WiFi.encryptionType(i)),
                sizeof(wifiSurveyEntries[i].security));
        wifiSurveyEntries[i].rssi = static_cast<int16_t>(WiFi.RSSI(i));
        wifiSurveyEntries[i].channel = static_cast<uint8_t>(WiFi.channel(i));
    }
    WiFi.scanDelete();

    if (!WiFi.mode(previousMode)) {
        LOG_ERROR("Ghost WiFi scan could not restore previous radio mode");
        screen->showSimpleBanner("WiFi scan finished\nRadio restore failed", 5000);
        return;
    }

    if (wifiSurveyCount == 0) {
        screen->showSimpleBanner("No WiFi networks found", 4000);
        return;
    }

    static char labels[kMaxWifiSurveyEntries][40];
    static const char *optionsArray[kMaxWifiSurveyEntries + 1];
    optionsArray[0] = "Back";
    for (uint8_t i = 0; i < wifiSurveyCount; ++i) {
        snprintf(labels[i], sizeof(labels[i]), "CH %u  %s", wifiSurveyEntries[i].channel, wifiSurveyEntries[i].ssid);
        optionsArray[i + 1] = labels[i];
    }

    BannerOverlayOptions bannerOptions;
    bannerOptions.message = "WiFi Survey (passive)";
    bannerOptions.optionsArrayPtr = optionsArray;
    bannerOptions.optionsCount = wifiSurveyCount + 1;
    bannerOptions.bannerCallback = [](int selected) -> void {
        if (selected <= 0 || selected > wifiSurveyCount) {
            return;
        }

        const WifiSurveyEntry &network = wifiSurveyEntries[selected - 1];
        char details[180];
        if (WiFi.status() == WL_CONNECTED) {
            snprintf(details, sizeof(details), "SSID: %s\nMAC: %s\nCH %u RSSI %d\n%s\nMy IP: %s",
                     network.ssid, network.bssid, network.channel, network.rssi, network.security,
                     WiFi.localIP().toString().c_str());
        } else {
            snprintf(details, sizeof(details), "SSID: %s\nMAC: %s\nCH %u RSSI %d\n%s\nAP IP not broadcast",
                     network.ssid, network.bssid, network.channel, network.rssi, network.security);
        }
        screen->showSimpleBanner(details, 8000);
    };
    screen->showOverlayBanner(bannerOptions);
}

void menuHandler::wifiNetworkInfoMenu()
{
    if (WiFi.status() != WL_CONNECTED) {
        screen->showSimpleBanner("Not connected to WiFi\nA scan cannot reveal AP IPs", 5000);
        return;
    }
    ...
    char details[180];
    snprintf(details, sizeof(details), "SSID: %s\nDevice MAC: %s\nDevice IP: %s\nGateway: %s",
             ssid.c_str(), mac.c_str(), ip.c_str(), gateway.c_str());
    screen->showSimpleBanner(details, 8000);
}
#endif

#if defined(M5STACK_CARDPUTER_ADV)
void menuHandler::volumePickerMenu()
{
    static const char *optionsArray[] = {"Back", "Mute", "25%", "50%", "75%", "100%"};
    static constexpr uint8_t levels[] = {0, 25, 50, 75, 100};
    const uint8_t volume = ghostAudio::getVolume();
    const uint8_t selected = volume < 13 ? 1 : volume < 38 ? 2 : volume < 63 ? 3 : volume < 88 ? 4 : 5;

    BannerOverlayOptions bannerOptions;
    bannerOptions.message = "Speaker Volume";
    bannerOptions.optionsArrayPtr = optionsArray;
    bannerOptions.optionsCount = 6;
    bannerOptions.InitialSelected = selected;
    bannerOptions.bannerCallback = [](int choice) -> void {
        if (choice <= 0 || choice > static_cast<int>(sizeof(levels) / sizeof(levels[0]))) {
            return;
        }
        if (!ghostAudio::setVolume(levels[choice - 1])) {
            LOG_ERROR("Ghost speaker volume changed but could not be saved");
            screen->showSimpleBanner("Volume applied\nCould not save setting", 5000);
            return;
        }
        screen->showSimpleBanner("Speaker volume saved", 2500);
    };
    screen->showOverlayBanner(bannerOptions);
}
#endif
...
void menuHandler::screenOptionsMenu()
{
    ...
    enum optionsNumbers { Back, Brightness, Volume, FrameToggles, DisplayUnits, MessageBubbles, Theme };
...
#if defined(M5STACK_CARDPUTER_ADV)
    optionsArray[options] = "Speaker Volume";
    optionsEnumArray[options++] = Volume;
#endif
    ...
    bannerOptions.bannerCallback = [](int selected) -> void {
        if (selected == Brightness) {
            ...
            menuHandler::menuQueue = menuHandler::VolumePicker;
            screen->runNow();
#endif
