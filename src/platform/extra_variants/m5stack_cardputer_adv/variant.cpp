...
namespace
{
Preferences ghostPreferences;
bool ghostPreferencesReady = false;
uint8_t ghostVolume = 75;
} // namespace
...
void lateInitVariant()
{
    ...
    board.begin(cfg);
    ghostPreferencesReady = ghostPreferences.begin("ghost", false);
    if (ghostPreferencesReady) {
        const uint8_t storedVolume = ghostPreferences.getUChar("volume", ghostVolume);
        if (storedVolume <= 100) {
            ghostVolume = storedVolume;
        } else {
            LOG_ERROR("Stored Ghost speaker volume is out of range: %u", storedVolume);
        }
    } else {
        LOG_ERROR("Could not open Ghost settings storage");
    }
    board.setVolume(ghostVolume);
...
{
    if (volume > 100) {
        LOG_ERROR("Invalid Ghost speaker volume: %u", volume);
        return false;
    }
    ...
    board.setVolume(ghostVolume);
    if (!ghostPreferencesReady) {
        LOG_ERROR("Ghost speaker volume applied but settings storage is unavailable");
        return false;
    }
    if (ghostPreferences.putUChar("volume", ghostVolume) != sizeof(ghostVolume)) {
        LOG_ERROR("Ghost speaker volume applied but could not be saved");
        return false;
    }
    return true;
}
